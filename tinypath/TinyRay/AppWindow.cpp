...
// Flags are used to switch between RayTrace and PathTrace
enum Trace {
	PATH_FLAG = 0x1,
	RAY_FLAG = 0x1 << 1,
};
Trace trace = PATH_FLAG;
...
BOOL AppWindow::InitWindow(HINSTANCE hInstance, int width, int height)
{
	...
	m_pPathTracer = new PathTracer(width, height);
	m_pScene = new Scene();
...
void AppWindow::Render()
{
	...
	// The condition for the flags 
	if (trace == PATH_FLAG)
	{
		Colour *pBuffer = m_pPathTracer->GetFramebuffer()->GetBuffer();
		m_pPathTracer->DoPathTrace(m_pScene);
		glDrawPixels(m_width, m_height, GL_RGBA, GL_FLOAT, pBuffer);
...
BOOL AppWindow::KeyUp(WPARAM key)
{
	...
	// The switch key is used to change the flags as well
	switch (key)
	{
	case VK_F1:
		trace = PATH_FLAG;
		m_pPathTracer->m_traceflag = PathTracer::TRACE_AMBIENT;
		break;
	case VK_F2:
		trace = RAY_FLAG;
	...
	case VK_F7:
		...
		break;
	case VK_F8:
		m_pPathTracer->m_traceflag = (PathTracer::TraceFlags)(PathTracer::TRACE_AMBIENT | PathTracer::TRACE_DIFFUSE_AND_SPEC
			| PathTracer::TRACE_REFRACTION | PathTracer::TRACE_REFLECTION | PathTracer::TRACE_SHADOW);
		break;
	...
	if (RAY_FLAG)
	{
		m_pPathTracer->ResetRenderCount();
	}
